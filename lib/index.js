...
function parseInput (data, queryStart, queryEnd, excludeMain, projectName) {
  ...
  Object.keys(data).forEach((v) => {
    ...
    const alpha = version.alpha ? new Date(version.alpha) : null;
    ...
    if (alpha !== null && alpha < queryEnd && current > queryStart) {
      output.push({ name, type: 'alpha', start: alpha, end: current });
    }
  });
